...
to_number(acc.CD_CATEGORIA) CD_CATEGORIA, 
 ...
 WHERE 
     ...
     and ( 
          a.dt_entrada BETWEEN 
                trunc(TO_DATE('01/10/2025', 'DD/MM/YYYY')) 
            and trunc(TO_DATE('31/10/2025', 'DD/MM/YYYY'))  + 0.99999 
 ...
 and ( 1=1 
 and ( contrato.nr_contrato in  (2444)  ) 
     ) 
 ), 
 ...
 outer apply( 
  ...
  -- fim coparticipacao 
	          where 1=1 
	             ...
	             -- Filtro para garantir que os procedimentos restornos são do mes atual. 
             and ( 
                  pr_pa.dt_procedimento BETWEEN trunc(TO_DATE('01/10/2025', 'DD/MM/YYYY'))  and trunc(TO_DATE('31/10/2025', 'DD/MM/YYYY')) + 0.99999             ) 
             and case 
             when 
  ...
  outer apply( 
	...
	SELECT a.*, 
	 ...
	 union all 
	SELECT c.*, ' ' as cid_doenca  from procedimentos_autorizados_pela_operadora c 
)